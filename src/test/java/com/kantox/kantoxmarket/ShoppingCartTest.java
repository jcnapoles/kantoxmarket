...
import com.kantox.kantoxmarket.shoppingcart.offers.BuyMoreXItemGetYDiscountFromTotalPrice;
import com.kantox.kantoxmarket.shoppingcart.offers.BuyMoreXItemGetYPercentDiscountEachProduct;
...
public class ShoppingCartTest {
	...
	@Before
	public void setup() {
		cart = ShoppingCart.builder().build();
		IOffer offerGR1 = BuyXItemGetYItemFreeOffer.builder().XItem(2).YItem(1).build();
		IOffer offerSR1 = BuyMoreXItemGetYPercentDiscountEachProduct.builder().XItem(3).Y(10.0).build();
		IOffer offerCF1 = BuyMoreXItemGetYDiscountFromTotalPrice.builder().XItem(3).build();
	...
	public void testAddSingleProductToShoppingCart() {
		...
		Assert.assertEquals(3.11, cart.checkout(), 0.001);
	}
	...
	public void addDifferentProductsToShoppingCart() {
		...
		Assert.assertEquals(19.34, cart.checkout(), 0.001);
	}
	...
	public void testAddMultipleQuantityOfAProductAndApplyOfferToShoppingCart() {
		...
		cart.addProduct(gr1);
		Assert.assertEquals(1, cart.getProductCount());
		Assert.assertEquals(9.33, cart.checkout(), 0.001);
	}
	...
	public void addDifferentProductsAndAppyOfferToShoppingCart() {
		...
		cart.addProduct(cf1);
		Assert.assertEquals(3, cart.getProductCount());
		Assert.assertEquals(27.45, cart.checkout(), 0.001);
	}

	@Test
	public void testApplyBuyXItemGetYPercentDiscountOnEachItemPrice() {
		IOffer offer = BuyMoreXItemGetYPercentDiscountEachProduct.builder().XItem(3).Y(10.0).build();
		ShoppingCart cart = ShoppingCart.builder().build();
		...
		Assert.assertEquals(13.50, cart.checkout(), 0.001);
	}
	...
	public void testApplyBuyMoreXItemGetYPercentDiscountOnEachProductPrice() {
		...
		Assert.assertEquals(18.00, cart.checkout(), 0.001);
	}

	@Test
	public void testApplyBuyXItemGetYDiscountOnOriginalItemPrice() {
		IOffer offer = BuyMoreXItemGetYDiscountFromTotalPrice.builder().XItem(3).build();
		ShoppingCart cart = ShoppingCart.builder().build();
		...
		Assert.assertEquals(22.46, cart.checkout(), 0.001);
	}
	...
	public void testApplyBuyMoreXItemGetTwoThirdsDiscountOnOriginalPrice() {
		...
		Assert.assertEquals(37.43, cart.checkout(), 0.001);
	}
	...
	public void addDiferentProductsAndApplyEachDiferentOffers() {
		...
		cart.addProduct(cf1);
		Assert.assertEquals(3, cart.getProductCount());
		Assert.assertEquals(64.76, cart.checkout(), 0.001);
	}
	...
	public void addDiferentProductsInAnyOrderAndApplyOneOffer() {
		...
		cart.addProduct(cf1);
		Assert.assertEquals(3, cart.getProductCount());
		Assert.assertEquals(22.45, cart.checkout(), 0.001);
	}
	...
	public void addDiferentProductsInAnyOrderAndApplyDiferentOffers() {
		...
		Assert.assertEquals(3, cart.getProductCount());
		Assert.assertEquals(42.18, cart.checkout(), 0.001);
	}
